...
```mermaid
graph TD
    Client["Application or psql<br/>connects to the coordinator only"]
    ...
    subgraph Host["One Docker host"]
        ...
        W1["<b>worker-1</b><br/>Four shards after the rebalance<br/>102010, 102014, 102016, 102018"]
        W2["<b>worker-2</b><br/>Four shards after the rebalance<br/>102009, 102015, 102017, 102019"]
        W3["<b>worker-3</b><br/>Added to the running cluster<br/>102008, 102011, 102012, 102013"]
    end
...
flowchart TD
    Client["Application"]
    Coord["Coordinator<br/>parses the SQL, hashes the filter,<br/>works out which shards are involved"]
    D{"Does the query filter<br/>on the shard key?"}
    ...
    Combine["Coordinator combines<br/>the partial results"]

    Client --> Coord --> D
    D -->|"Yes. One shard,<br/>so one worker."| W1
    D -->|"No. Every shard,<br/>so every worker."| W2
    D -.-> W3
    W1 --> Combine
    W2 --> Combine
    W3 --> Combine
...
```mermaid
flowchart LR
    Row["A row arrives with<br/><b>customer_id = 42</b>"]
    Hash["The value is hashed.<br/>The 32 bit hash range is split<br/>into 12 equal slices,<br/>one per shard."]
    Node["The value lands in the slice<br/>owned by <b>shard 102018</b>,<br/>which lives on <b>worker-1</b>,<br/>so the row goes there."]

    Row --> Hash --> Node

    style Row fill:#607D8B,color:#fff
    style Hash fill:#FF9800,color:#fff
    style Node fill:#4CAF50,color:#fff
```
...
```mermaid
flowchart TD
    subgraph SG["One shard group, on one worker"]
        C["customers_102008"]
        O["orders_102020"]
        I["order_items_102032"]
    end

    Note["A customer, their orders and<br/>their order items all hash to the<br/>same slice, so they share a worker.<br/>Joins between them never leave it."]
    ...
    style Note fill:#ECEFF1,color:#000
```
...
![Rows in each of the twelve shards, with the four that later moved to worker-3 highlighted](charts/rows-per-shard.png)
...
![Shards per worker before and after the rebalance, going from six and six to four, four and four](charts/shards-per-worker.png)
...
![Customer rows per worker before and after the rebalance](charts/rows-per-worker.png)
...
![Total rows per table, identical before and after the rebalance](charts/row-totals.png)
...
sequenceDiagram
    ...
    Note over Src,Dst: copying, shard stays readable
    Src->>Dst: copy all existing rows
    Src->>Dst: stream any ongoing changes

    Note over Coord: cutover
    ...
    Note over Coord: queries now route to the destination
```
...
```mermaid
flowchart TD
    F1["Coordinator fails"]
    F2["worker-1 fails"]
    F3["worker-2 fails"]
    F4["worker-3 fails"]

    R1["Everything fails.<br/>It is the only entry point,<br/>and it holds the shard map."]
    R2["Its four shards are gone.<br/>Queries needing them fail.<br/>Others still work."]
...
graph TD
    App["Application"]
    HAP["HAProxy<br/>stable address for the coordinator"]

    subgraph GC["Coordinator, replicated"]
        C1["coordinator<br/><b>primary</b>"]
        C2["replica"]
        C3["replica"]
    end

    subgraph GW1["worker-1, replicated"]
        W1["<b>primary</b><br/>4 shards"]
        W1b["replica"]
        W1c["replica"]
    end

    subgraph GW2["worker-2, replicated"]
        W2["<b>primary</b><br/>4 shards"]
        W2b["replica"]
        W2c["replica"]
    end

    subgraph GW3["worker-3, replicated"]
        W3["<b>primary</b><br/>4 shards"]
        W3b["replica"]
        W3c["replica"]
    end
