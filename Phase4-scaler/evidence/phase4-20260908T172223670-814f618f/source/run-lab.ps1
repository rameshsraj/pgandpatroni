...
function Runtime([string]$Line) {
    ...
    if ($Line -ne 'show stat' -and -not [string]::IsNullOrWhiteSpace($r.stdout)) {
...
function Stop-Traffic {
    ...
    $name = $script:traffic
    $r = Cmd -Argv @('stop','--signal','SIGTERM','--time','10',$name) -AllowFailure
...
try {
    ...
    foreach ($file in @('run-lab.ps1','traffic.sh','read.sql','seed.sql','haproxy.cfg','patroni-template.yml','README.md')) {
        Save-Text (Join-Path $source $file) ([IO.File]::ReadAllText((Join-Path $root $file)).Replace("`r`n","`n"))
    }
