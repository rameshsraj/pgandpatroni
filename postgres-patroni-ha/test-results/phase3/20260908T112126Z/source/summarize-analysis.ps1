...
# PowerShell 7.6 converts ISO JSON strings into DateTime automatically. Normalize
# them back to UTC ISO strings before CSV/Markdown formatting (no locale dates).
foreach ($record in @($events + $bench + $snaps + $writes + $reads + $commands)) {
    foreach ($field in @('utc','start','end')) {
        if ($record.PSObject.Properties[$field] -and $record.$field -is [DateTime]) {
...
Copy-Item $PSCommandPath "$dir/source/summarize-analysis.ps1" -Force
