...
try {
    ...
    Event preparation-end 'Topology remains four nodes; elastic image prebuilt. First run clones; later runs reuse the preserved elastic volume.'
    ...
    RunStage '03-auto-scale-out' 65 120 12 {
        ...
        if (-not $script:ScaleStarted) {
            ...
            if ($n.Count -and $n[0].State -eq 'streaming' -and $n[0].'Lag in MB' -eq 0 -and -not $script:SlotEnabled) {
                $ready = Sql 'pg-node-5' 'SELECT count(*) FROM phase3_lab.items;' -AllowFailure
                if ($ready.exit -eq 0 -and $ready.stdout.Trim() -eq '100000') {
                    Runtime 'set server pg-replicas/pg-node-5 state ready'
                    Runtime 'set server pg-primary/pg-node-5 state ready'
                    $script:SlotEnabled=$true
                    Event elastic-slot-admitted 'SQL ready, streaming, reported lag zero; runtime admission, no reload'
                }
            }
            $s = @(ProxyStats)
    ...
    RunStage '07-rejoin-under-load' 60 120 12 {
        ...
        if (-not $script:Restored) {
            ...
            # Quarantine a returning server during SQL recovery, not just DNS
            # resolution. The controller releases it after observed readiness.
            Runtime "set server pg-replicas/$($script:Killed) state maint"
            Runtime "set server pg-primary/$($script:Killed) state maint"
            ...
            if ($n.Count -and $n[0].Role -eq 'Replica' -and $n[0].State -eq 'streaming' -and $n[0].'Lag in MB' -eq 0) {
                $ready = Sql $script:Killed 'SELECT count(*) FROM phase3_lab.items;' -AllowFailure
                if ($ready.exit -ne 0 -or $ready.stdout.Trim() -ne '100000') { return }
                Runtime "set server pg-replicas/$($script:Killed) state ready"
                Runtime "set server pg-primary/$($script:Killed) state ready"
    ...
    RunStage '09-auto-scale-in' 40 5 2 {
        ...
        if (-not $script:ScaledIn) {
            ...
            if ($script:LowCount -ge 2) {
                ...
                Runtime 'set server pg-replicas/pg-node-5 state maint'
                ...
                # Remain in maintenance: enabling immediately after stop can
                # route new sessions using the last cached healthy check result.
                # A subsequent scale-out admits it only after SQL readiness.
                $script:ScaledIn=$true
                Event scale-in-complete 'pg-node-5 stopped; slot in maintenance; volume preserved; no proxy restart'
            }
        }
    }
